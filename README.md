...
```bash
apt update && apt install -y curl git && curl -fsSL https://get.docker.com | sh && git clone https://github.com/Lukas0923xcv/SSH-Center.git /opt/webssh && cd /opt/webssh && mkdir -p data && chmod +x launcher.sh && docker compose up -d --build
```
...
```bash
git clone https://github.com/Lukas0923xcv/SSH-Center.git /opt/webssh && cd /opt/webssh && mkdir -p data && chmod +x launcher.sh && docker compose up -d --build
```
